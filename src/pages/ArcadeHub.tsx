...
export default function ArcadeHub({ theme: t, user, onLogout, onAvatarChange }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: "'Outfit', sans-serif" }}>
      {/* Header */}
      <div style={{ background: t.surface, borderBottom: `1px solid ${t.border}`, backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 100, padding: '0 clamp(12px,4vw,40px)' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: 'clamp(16px,4vw,32px) clamp(12px,4vw,40px)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '28px', textAlign: 'center' }}>
          <h1 style={{ color: t.text, fontWeight: 900, fontSize: 'clamp(1.6rem,5vw,2.8rem)', margin: '0 0 6px', letterSpacing: '-0.03em' }}>
            {user ? `Welcome back, ${user.avatar} ${user.username}!` : '🦊 Foxy Arcade'}
          </h1>
        ...
        >
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search games…"
            style={{ width: '100%', background: t.surface, border: `2px solid ${t.border}`, borderRadius: '14px', padding: '12px 14px 12px 42px', color: t.text, fontSize: '0.95rem', outline: 'none', transition: 'border-color 0.2s', boxSizing: 'border-box' }}
        ...
        >
          {TAGS.map(tag => (
            <button key={tag} onClick={() => setActiveTag(tag)}
              style={{ background: activeTag===tag?t.accent:t.surface, border: `1px solid ${activeTag===tag?t.accent:t.border}`, borderRadius: '20px', padding: '6px 16px', cursor: 'pointer', color: activeTag===tag?'#fff':t.textMuted, fontWeight: 600, fontSize: '0.82rem', textTransform: 'capitalize', transition: 'all 0.2s', backdropFilter: 'blur(8px)', fontFamily: 'Outfit,sans-serif' }}
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', gap: '4px' }}>
        {game.hot && <span style={{ background: '#ef4444', color: '#fff', borderRadius: '8px', padding: '2px 6px', fontSize: '0.58rem', fontWeight: 800 }}>🔥 HOT</span>}
        {game.isNew && <span style={{ background: game.color, color: '#fff', borderRadius: '8px', padding: '2px 6px', fontSize: '0.58rem', fontWeight: 800 }}>NEW</span>}
      </div>
      <motion.div animate={{ scale: hovered ? 1.15 : 1 }} transition={{ duration: 0.25 }}
        style={{ fontSize: 'clamp(1.8rem,4vw,2.4rem)', marginBottom: '10px', display: 'block' }}
      ...
      <div style={{ color: t.textMuted, fontSize: '0.72rem', marginBottom: '10px', lineHeight: 1.4 }}>{game.description}</div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ background: `${game.color}22`, color: game.color, borderRadius: '8px', padding: '3px 8px', fontSize: '0.66rem', fontWeight: 700 }}>👥 {game.players}</span>
