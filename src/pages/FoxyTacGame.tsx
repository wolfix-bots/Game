...
import { Settings2, RotateCcw, Bot, Users, Globe, Trophy, TrendingUp, Calendar, ArrowLeft } from 'lucide-react';
import '../styles.css';
...
export default function FoxyTacGame({ user, onLogout, onAvatarChange }: Props) {
  ...
  const handleGameOver = useCallback((newBoard: string[], w: string | null, d: boolean, moves: number) => {
    if (w) {
      ...
      if (gameMode === 'ai' && w === 'X') { tryUnlock(`beat_${aiDifficulty}`); }
    ...
    <div style={{ minHeight: '100vh', background: themeConfig.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'clamp(12px,3vw,24px)', fontFamily: "'Outfit',sans-serif" }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button onClick={() => nav('/')} style={{ background: themeConfig.surface, border: `1px solid ${themeConfig.border}`, borderRadius: '10px', padding: '7px', cursor: 'pointer', color: themeConfig.textMuted, display: 'flex' }}>
            <ArrowLeft size={16} />
          </button>
          ...
          >🦊</motion.div>
          <div>
            <h1 style={{ color: themeConfig.text, fontWeight: 900, fontSize: 'clamp(1rem,4vw,1.3rem)', margin: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          {user ? <UserMenu user={user} theme={themeConfig} onLogout={onLogout} onAvatarChange={a => onAvatarChange({ ...user, avatar: a })} /> : <div style={{ background: themeConfig.cellBg, border: `1px solid ${themeConfig.border}`, borderRadius: '10px', padding: '5px 10px', color: themeConfig.textMuted, fontSize: '0.75rem', fontWeight: 600 }}>👤 Guest</div>}
          <button onClick={() => setDailyOpen(true)} style={{ background: dailyDone?'#22c55e22':`${themeConfig.accent}22`, border: `1px solid ${dailyDone?'#22c55e44':themeConfig.accent+'44'}`, borderRadius: '10px', padding: '7px', cursor: 'pointer', color: dailyDone?'#22c55e':themeConfig.accent, display: 'flex' }}><Calendar size={15} /></button>
