...
export default function TicTacToePage({ user, onLogout, onAvatarChange }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: themeConfig.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'clamp(12px, 3vw, 24px)', fontFamily: "'Outfit', sans-serif" }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div>
            ...
            {stats.currentStreak >= 2 && <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} style={{ fontSize: '0.62rem', color: '#f97316', fontWeight: 700 }}>🔥 {stats.currentStreak} streak!</motion.div>}
          </div>
        </div>

        <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          {user ? (
            <UserMenu user={user} theme={themeConfig} onLogout={onLogout} onAvatarChange={a => onAvatarChange({ ...user, avatar: a })} />
          ) : (
      ...
      <SettingsPanel open={settingsOpen} onClose={() => setSettingsOpen(false)} theme={theme} themeConfig={themeConfig}
        onThemeChange={t => { setTheme(t); if (t === 'night') tryUnlock('night_owl'); if (t === 'neon') tryUnlock('neon_rider'); }}
        emojiSet={emojiSetIndex} onEmojiChange={setEmojiSetIndex} soundEnabled={soundEnabled} onSoundToggle={() => setSoundEnabled(s => !s)} />
