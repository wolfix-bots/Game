...
export default function TicTacToePage({ user: propUser }: Props) {
  ...
  const foxTimer = useRef<any>(null);
  ...
  const handleGameOver = useCallback((newBoard: string[], w: string | null, d: boolean, moves: number) => {
    if (w) {
      ...
      if (gameMode === 'ai' && w === 'X') {
        if (aiDifficulty === 'easy') tryUnlock('beat_easy');
        if (aiDifficulty === 'medium') tryUnlock('beat_medium');
        if (aiDifficulty === 'hard') tryUnlock('beat_hard');
      }
      if (user) submitScore('tictactoe', user.username, user.avatar, stats.totalWins * 10);
    } else if (d) {
  ...
  }, [gameMode, scoreMode, soundEnabled, aiDifficulty, theme, foxSay, tryUnlock, user]);
    ...
    <div style={{ minHeight: '100vh', background: themeConfig.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'clamp(12px,3vw,24px)', fontFamily: "'Outfit',sans-serif" }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button onClick={() => nav('/')} style={{ background: themeConfig.surface, border: `1px solid ${themeConfig.border}`, borderRadius: '10px', padding: '8px', cursor: 'pointer', color: themeConfig.textMuted, display: 'flex', backdropFilter: 'blur(8px)' }}>
            ←
          </button>
          <motion.div animate={{ rotate: [0,8,-8,0] }} transition={{ repeat: Infinity, duration: 3 }}
            style={{ background: isNeon ? 'transparent' : themeConfig.accent, border: isNeon ? `2px solid ${themeConfig.accent}` : 'none', borderRadius: '12px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}
          >🦊</motion.div>
          <div>
            <h1 style={{ color: themeConfig.text, fontWeight: 900, fontSize: 'clamp(1rem,4vw,1.3rem)', margin: 0, letterSpacing: '-0.02em' }}>
              Foxy<span style={{ color: themeConfig.accent }}>Tac</span>
            </h1>
            {stats.currentStreak >= 2 && <div style={{ fontSize: '0.62rem', color: '#f97316', fontWeight: 700 }}>🔥 {stats.currentStreak} streak!</div>}
          </div>
        </div>
        <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          {user ? <UserMenu user={user} theme={themeConfig} onLogout={() => { logout(); setUser(null); }} onAvatarChange={a => setUser(u => u ? { ...u, avatar: a } : u)} />
            : <div style={{ background: themeConfig.cellBg, border: `1px solid ${themeConfig.border}`, borderRadius: '10px', padding: '5px 10px', color: themeConfig.textMuted, fontSize: '0.75rem', fontWeight: 600 }}>👤 Guest</div>}
          {[
            { icon: <Calendar size={15} />, onClick: () => setDailyOpen(true), active: dailyDone, color: dailyDone ? '#22c55e' : themeConfig.accent, title: 'Daily Puzzle' },
            { icon: <Trophy size={15} />, onClick: () => setAchievementsOpen(true), active: false, color: themeConfig.textMuted, title: 'Achievements' },
            { icon: <TrendingUp size={15} />, onClick: () => setStatsOpen(true), active: false, color: themeConfig.textMuted, title: 'Stats' },
            { icon: <Settings2 size={15} />, onClick: () => setSettingsOpen(true), active: false, color: themeConfig.textMuted, title: 'Settings' },
          ].map((btn, i) => (
            <button key={i} onClick={btn.onClick} title={btn.title}
              style={{ background: themeConfig.surface, border: `1px solid ${btn.active ? btn.color + '44' : themeConfig.border}`, borderRadius: '10px', padding: '7px', cursor: 'pointer', color: btn.active ? btn.color : themeConfig.textMuted, display: 'flex', backdropFilter: 'blur(8px)', transition: 'all 0.2s' }}
            >{btn.icon}</button>
          ))}
        </div>
      </motion.header>
      ...
      >
        ...
        <div style={{ background: themeConfig.surface, border: `1px solid ${themeConfig.border}`, borderRadius: '16px', padding: '6px', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '4px', backdropFilter: 'blur(12px)', boxShadow: themeConfig.shadow }}>
          {([['local','👥 Local',<Users size={13}/>],['ai','🤖 vs AI',<Bot size={13}/>],['online','🌐 Online',<Globe size={13}/>]] as const).map(([mode,label,icon]) => (
            <button key={mode} onClick={() => { setGameMode(mode as GameMode); resetGame(); }}
              style={{ background: gameMode === mode ? themeConfig.accent : 'transparent', border: 'none', borderRadius: '10px', padding: '8px 4px', cursor: 'pointer', color: gameMode === mode ? '#fff' : themeConfig.textMuted, fontWeight: 700, fontSize: '0.78rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', transition: 'all 0.2s', fontFamily: 'Outfit,sans-serif' }}
            >{icon}{label}</button>
          ))}
        </div>
        ...
        {gameMode === 'online' ? (
          ...
          <>
            ...
            <div style={{ background: themeConfig.surface, border: `1px solid ${themeConfig.border}`, borderRadius: '14px', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backdropFilter: 'blur(12px)', boxShadow: themeConfig.shadow }}>
              <motion.div key={turnLabel()} initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} style={{ color: winner ? '#22c55e' : draw ? '#f59e0b' : themeConfig.text, fontWeight: 700, fontSize: 'clamp(0.88rem,3vw,1rem)' }}>
                {turnLabel()}
              </motion.div>
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                {moveCount > 0 && !winner && !draw && <span style={{ color: themeConfig.textMuted, fontSize: '0.72rem', fontWeight: 600, background: themeConfig.cellBg, borderRadius: '8px', padding: '3px 8px' }}>Move {moveCount}</span>}
                <button onClick={resetGame} style={{ background: themeConfig.cellBg, border: `1px solid ${themeConfig.border}`, borderRadius: '10px', padding: '7px', cursor: 'pointer', color: themeConfig.textMuted, display: 'flex', transition: 'all 0.2s' }} title="New Game">
                  <RotateCcw size={15} />
                </button>
              </div>
            </div>
            ...
            <AnimatePresence>
              {(winner || draw) && (
                ...
                >
                  ...
                  <button onClick={resetGame} style={{ background: themeConfig.accent, border: 'none', borderRadius: '10px', padding: '8px 20px', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem', display: 'inline-flex', alignItems: 'center', gap: '6px', fontFamily: 'Outfit,sans-serif' }}>
      ...
      <Leaderboard open={lbOpen} onClose={() => setLbOpen(false)} theme={themeConfig} gameId="tictactoe" />
    </div>
  );
}
