...
export default function TicTacToePage({ user }: { user: User | null }) {
  ...
  const handleGameOver = useCallback((newBoard: string[], w: string | null, d: boolean, moves: number) => {
    if (w) {
      ...
      if (gameMode === 'ai') foxSay(w === 'O' ? getCommentary('win') : getCommentary('lose'), w === 'O' ? 'excited' : 'sad');
  ...
  return (
    <div style={{ minHeight: '100vh', background: themeConfig.bg, fontFamily: "'Outfit', sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: themeConfig.surface, borderBottom: `1px solid ${themeConfig.border}`, backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 100, padding: '0 16px' }}>
        <div style={{ maxWidth: '520px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '56px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <motion.span animate={{ rotate: [0,8,-8,0] }} transition={{ repeat: Infinity, duration: 3 }} style={{ fontSize: '1.5rem' }}>🦊</motion.span>
            <div>
              <span style={{ color: themeConfig.text, fontWeight: 900, fontSize: '1.1rem' }}>Foxy</span>
              <span style={{ color: themeConfig.accent, fontWeight: 900, fontSize: '1.1rem' }}>Tac</span>
              {stats.currentStreak >= 2 && <div style={{ fontSize: '0.6rem', color: '#f97316', fontWeight: 700 }}>🔥 {stats.currentStreak} streak!</div>}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
            {user && <UserMenu user={user} theme={themeConfig} onLogout={() => {}} onAvatarChange={() => {}} />}
            <button onClick={() => setDailyOpen(true)} style={{ background: dailyDone?'#22c55e22':`${themeConfig.accent}22`, border: `1px solid ${dailyDone?'#22c55e44':themeConfig.accent+'44'}`, borderRadius:'10px',padding:'7px',cursor:'pointer',color:dailyDone?'#22c55e':themeConfig.accent,display:'flex' }} title="Daily"><Calendar size={15}/></button>
            <button onClick={() => setAchievementsOpen(true)} style={{ background:themeConfig.surface,border:`1px solid ${themeConfig.border}`,borderRadius:'10px',padding:'7px',cursor:'pointer',color:themeConfig.textMuted,display:'flex' }}><Trophy size={15}/></button>
            <button onClick={() => setStatsOpen(true)} style={{ background:themeConfig.surface,border:`1px solid ${themeConfig.border}`,borderRadius:'10px',padding:'7px',cursor:'pointer',color:themeConfig.textMuted,display:'flex' }}><TrendingUp size={15}/></button>
            <button onClick={() => setSettingsOpen(true)} style={{ background:themeConfig.surface,border:`1px solid ${themeConfig.border}`,borderRadius:'10px',padding:'7px',cursor:'pointer',color:themeConfig.textMuted,display:'flex' }}><Settings2 size={15}/></button>
          </div>
        </div>
      </div>

      <div style={{ maxWidth: '480px', margin: '0 auto', padding: 'clamp(12px,3vw,20px)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {/* Mode tabs */}
        <div style={{ background:themeConfig.surface,border:`1px solid ${themeConfig.border}`,borderRadius:'16px',padding:'6px',display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'4px',backdropFilter:'blur(12px)' }}>
        ...
        {gameMode === 'online' ? (
          ...
          <>
            {gameMode === 'ai' && (
              <div style={{background:themeConfig.surface,border:`1px solid ${themeConfig.border}`,borderRadius:'16px',padding:'12px 16px',backdropFilter:'blur(12px)'}}>
                <FoxMascot mood={foxMood} message={foxMsg} accentColor={themeConfig.accent} />
              </div>
            )}
            <div style={{background:themeConfig.surface,border:`1px solid ${themeConfig.border}`,borderRadius:'14px',padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',backdropFilter:'blur(12px)'}}>
              <motion.div key={turnLabel()} initial={{x:-10,opacity:0}} animate={{x:0,opacity:1}} style={{color:winner?'#22c55e':draw?'#f59e0b':themeConfig.text,fontWeight:700,fontSize:'clamp(0.9rem,3vw,1.05rem)'}}>{turnLabel()}</motion.div>
              <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
                ...
                <button onClick={resetGame} style={{background:themeConfig.cellBg,border:`1px solid ${themeConfig.border}`,borderRadius:'10px',padding:'7px',cursor:'pointer',color:themeConfig.textMuted,display:'flex'}}><RotateCcw size={16}/></button>
              </div>
            </div>
            ...
            <AnimatePresence>
              {(winner||draw)&&(
                <motion.div initial={{scale:0.8,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.8,opacity:0}} style={{background:winner?'#22c55e22':'#f59e0b22',border:`1px solid ${winner?'#22c55e':'#f59e0b'}44`,borderRadius:'16px',padding:'16px',textAlign:'center'}}>
                  ...
                  <button onClick={resetGame} style={{background:themeConfig.accent,border:'none',borderRadius:'10px',padding:'8px 20px',color:'#fff',fontWeight:700,cursor:'pointer',fontSize:'0.9rem',display:'inline-flex',alignItems:'center',gap:'6px'}}><RotateCcw size={14}/> Play Again</button>
                </motion.div>
              )}
            </AnimatePresence>
