...
export default function Connect4() {
  ...
  const drop=useCallback((col:number)=>{
    if(winner) return;
    const r=getRow(board,col); if(r<0) return;
    ...
    const next=turn===1?2:1; setTurn(next);
    if(mode==='ai'&&next===2){
      setTimeout(()=>{
        const ac=aiMove(nb); const ar=getRow(nb,ac);
        const ab=nb.map(row=>[...row]); ab[ar][ac]=2;
        setBoard(ab);
  ...
  return (
    <GameShell title="Connect Four" emoji="🔴" onReset={reset} scores={[
      ...
      <div style={{textAlign:'center',marginBottom:'12px',fontWeight:700,fontSize:'1rem',
        color:winner===1?'#ef4444':winner===2?'#fbbf24':winner===-1?'#94a3b8':turn===1?'#ef4444':'#fbbf24'}}>
      ...
      <div style={{display:'inline-block',background:'#1e40af',borderRadius:'16px',padding:'10px',boxShadow:'0 8px 32px rgba(30,64,175,0.4)',overflowX:'auto',maxWidth:'100%'}}>
        ...
        {board.map((row,r)=>(
          <div key={r} style={{display:'flex',gap:'4px',marginBottom:r<ROWS-1?'4px':0}}>
            {row.map((cell,c)=>(
              <div key={c} onClick={()=>drop(c)} onMouseEnter={()=>setHover(c)} onMouseLeave={()=>setHover(-1)}
                style={{width:52,height:52,borderRadius:'50%',background:'#1e3a8a',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <AnimatePresence>
                  {cell!==0&&(
