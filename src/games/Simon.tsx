...
export default function Simon() {
  ...
  const reset = () => { setPhase('idle'); setSequence([]); setPlayerSeq([]); setScore(0); setActive(null); };
  ...
  return (
    <GameShell title="Simon Says" emoji="🔴" onReset={reset} scores={[
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', maxWidth:'260px', margin:'0 auto 20px' }}>
        {COLORS.map((color, i) => (
          <motion.button key={i}
            animate={{ scale: active===i?1.1:1, opacity: active===i?1:phase==='showing'?0.35:1, boxShadow: active===i?`0 0 30px ${color}`:'none' }}
            transition={{ duration: 0.1 }}
            onClick={() => press(i)}
            style={{ height:100, borderRadius:'20px', border:`3px solid ${color}55`, background: active===i?color:`${color}33`, cursor: phase==='input'?'pointer':'default', fontSize:'2rem' }}
